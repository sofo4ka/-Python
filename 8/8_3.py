...
class MyError(ValueError):
    ...
    @staticmethod
    def checking_element(input_value):
        try:
            if float(input_value):
                return True
        except ValueError:
            print("Ошибка! Элемент не является числом!!!")


list_of_numbers = []
input_data = 0
while True:
    input_data = input('Введите значения списка по одному элементу, введите stop для выхода: ')
    if input_data == 'stop':
        break
    elif MyError.checking_element(input_data):
